...
import {Pedido, PedidoConCliente, LineaPedido, PedidoCompletoCreateDto} from '../../models/pedido.model';
...
export class PedidosEntregadosComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.sub = this.pedidoSrv.getPedidosPorEstados(['Recepcionado', 'Pagado'])
      .pipe(
  ...
  direccionPedido: pedido.direccion || '—sin dirección—'
      ...
      .subscribe({
        ...
        error: err => {
          console.error(err);
          this.error = 'Error al cargar pedidos entregados';
          this.loading = false;
        }
      });
  }
